    ...
    private boolean solveH(int row, int col, int moveNumber) {
        if (moveNumber == board.length * board[0].length + 1) { //base case when you have completed tour
            ...
            if (row < 0 || row >= board.length ||
                ...
                boolean toReturn = moveNumber == board.length * board[0].length;
                /*
                Hellow, Mr. KONSTANTINOVICH. Here is the reason I set toReturn to this thingy instead of
                true: there is a situation in my code where at the very last step, options will have length
                0. However, this will cause the entire thing to be false, since the for loop below will not run.
                So I accomodated this situation using this weird boolean thingy.
                */
